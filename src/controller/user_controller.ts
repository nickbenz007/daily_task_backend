...
export const createUser = async (request: Request, response: Response) => {
    try {
        ...
        const token = jwt.sign({ email, name }, "token", { expiresIn: "7d" });
        ...
        return response.status(201).send({ message: "User has been created successfully", user, token });
    } catch (error) {
...
export const loginUser = async (request: Request, response: Response) => {
    try {
        ...
        if(!isPasswordIdentical){
            return response.status(401).send({message: 'Incorrect password please try again'})
        }

        const token = jwt.sign({email: existingUser.email, name: existingUser.name}, 'token', {expiresIn: '7d'})
        return response.send({message: 'Login successful', name: existingUser.name, email: existingUser.email, token} )
    }catch (error){
